...
//////////
// QuizHUD multiple choice question editing form.
// Can be used to edit an existing multiple choice question, or add a new one.
...
if (isset($_POST['save_question'])) {
    ...
    // Extract and sanitize our incoming data
    ...
    if ($_POST['id'] == 0) {
        ...
        $result = mysql_query("
            ...
            VALUES ({$formdata_quizid}, '{$formdata_text}', {$formdata_weight}, 'multiplechoice')
        ");
        if ($result) {
    ...
    header('Location: '.QUIZHUD_WWW_ROOT."/browser/question_multichoice.php?quizid={$quizid}&questionid={$questionid}");
    exit();
}
...
if (isset($_POST['save_answers'])) {
    ...
    // The answer data will be provided as 'text_x' and 'value_x', where 'x' is the answer ID.
    ...
    foreach ($_POST as $parname => $parvalue) {
        ...
        // Extract the ID, and find the 'value' parameter
...
if (isset($_POST['add_answer'])) {
    ...
    $result = mysql_query("
        ...
        VALUES ({$questionid}, 'z', '{$text}', {$value})
    ");
    if ($result) {
        // Re-structure the answers to accomodate the new one
        structure_answers($quizid, $questionid);
...
if ($questionid == 'new') {
	...
	if ($question->type != 'multiplechoice') error("Cannot edit question of type '{$question->type}' on this page.");
    
    // Have we been asked to move an answer?
    $moveanswerup = fetch_param_integer('moveanswerup', 0);
    $moveanswerdown = fetch_param_integer('moveanswerdown', 0);
    if ($moveanswerup > 0) {
        // Re-structure our answers
        if (!structure_answers($quizid, $questionid, $moveanswerup, true)) $_SESSION['errormessage'] = 'Failed to move answer:<br/>'.mysql_error();
        // Reload the page to eliminate the request parameter
        header('Location: '.QUIZHUD_WWW_ROOT."/browser/question_multichoice.php?quizid={$quizid}&questionid={$questionid}");
        exit();
        
    } else if ($moveanswerdown > 0) {
        // Re-structure our answers
        if (!structure_answers($quizid, $questionid, $moveanswerdown, false)) $_SESSION['errormessage'] = 'Failed to move answer:<br/>'.mysql_error();
        // Reload the page to eliminate the request parameter
        header('Location: '.QUIZHUD_WWW_ROOT."/browser/question_multichoice.php?quizid={$quizid}&questionid={$questionid}");
        exit();
    }
        ...
        if (!$deleteanswer) {
            ...
            $confirmmessage = <<<XXXEODXXX
             <p class="error">
              ...
              <a class="button" href="question_multichoice.php?quizid={$quizid}&amp;questionid={$questionid}&amp;deleteanswer={$deleteanswerid}&amp;confirmdelete=true" title="Click here to confirm the deletion."><span class="yes">Yes</span></a>
              &nbsp;
              <a class="button" href="question_multichoice.php?quizid={$quizid}&amp;questionid={$questionid}" title="Click here to cancel the deletion."><span class="no">No</span></a>
             </p>
XXXEODXXX;
        }
    }
...
case 'answer':
    ...
    <p>
     ...
     Please note that you cannot manually specify the 'shortname' -- this is determined automatically by the order of answers.
     You can re-order them by clicking the arrows at the side, but make sure not to do this before saving any changes you have made!
    </p>
XXXEODXXX;
	break;
...
case 'addanswer':
    ...
    <p>
	 ...
	 Initially, it will be appended to the existing answers, but you can re-order them later.
...
default:
    ...
    <p>
	 ...
	 Click one of the edit icons to edit either the question or answer details,
	  or click one of the arrows to re-order the answers.
...
if ($editmode == 'question') {
	// Editing form
	?>
	<form action="question_multichoice.php" method="post"><div>
	...
	else echo '<a href="',QUIZHUD_WWW_ROOT,'/browser/question_multichoice.php?quizid=',$quiz->id,'&amp;questionid=',$question->id,'" title="Click here to cancel edit mode.">Cancel</a>';
	...
	echo "Type: <b>Multiple Choice</b><br/><br/>\n";
	
	// Add an editing link
	echo '<a class="button" href="',QUIZHUD_WWW_ROOT,'/browser/question_multichoice.php?quizid=',$quiz->id,'&amp;questionid=',$question->id,'&amp;edit=question" title="Click here to edit the question."><span class="edit">Edit Question</span></a>';
}
?>
</div>
...
if ($answers === false) {
	...
	if ($editmode == 'answer') {
		echo '<form action="question_multichoice.php" method="post"><div>';
    ...
    foreach ($answers as $answer) {
        ...
        // Add a "delete" button
        echo "<a href=\"question_multichoice.php?quizid={$quiz->id}&amp;questionid={$question->id}&amp;deleteanswer={$answer->id}\" title=\"Click here to delete this answer.\"><img src=\"img/delete.png\" alt=\"delete\" /></a>";
        
        // Add a "move up" button
        if ($answernum > 0) {
        	echo "<a href=\"question_multichoice.php?quizid={$quiz->id}&amp;questionid={$question->id}&amp;moveanswerup={$answer->id}\" title=\"Click here to move this answer up one space.\"><img src=\"img/arrow_up.png\" alt=\"up\" /></a>";
        } else {
        	// Keep the buttons spaced out properly
            echo "<span>&nbsp;&nbsp;</span>\n";
        }
        
        // Add a "move down" button
        if ($answernum < ($numanswers - 1)) {
        	echo "<a href=\"question_multichoice.php?quizid={$quiz->id}&amp;questionid={$question->id}&amp;moveanswerdown={$answer->id}\" title=\"Click here to move this answer down one space.\"><img src=\"img/arrow_down.png\" alt=\"down\" /></a>";
        } else {
	        // Keep the buttons spaced out properly
            echo "<span>&nbsp;&nbsp;</span>\n";
        }
    ...
    if ($editmode == 'answer') {
		...
		// Cancel link
		echo '<a href="',QUIZHUD_WWW_ROOT,'/browser/question_multichoice.php?quizid=',$quiz->id,'&amp;questionid=',$question->id,'" title="Click here to cancel edit mode.">Cancel</a>';
    
    	echo "</div></form>";
    	
   	} else {
   		echo '<a class="button" href="',QUIZHUD_WWW_ROOT,'/browser/question_multichoice.php?quizid=',$quiz->id,'&amp;questionid=',$question->id,'&amp;edit=answer" title="Click here to edit the answers."><span class="edit">Edit Answers</span></a>';
   	}
}
?>
<br/><br/>
...
if ($editmode == 'addanswer') {
	...
	<label for="text">Text: </label>
	<input type="text" name="text" id="text" size="30" maxlength="255" value="" />&nbsp;
	...
	<a href="question_multichoice.php?quizid=<?php echo $quiz->id; ?>&amp;questionid=<?php echo $question->id; ?>" title="Click here to cancel this new answer.">Cancel</a>
	...
	?>
	<a class="button" href="question_multichoice.php?quizid=<?php echo $quiz->id; ?>&amp;questionid=<?php echo $question->id; ?>&amp;edit=addanswer#addanswer" title="Click here to add a new answer to this question."><span class="addanswer">Add Answer</span></a>
	<?php
}
